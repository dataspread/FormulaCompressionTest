...
declare -a spreadsheetString=("customsheet")
declare -a sheetPaths=("./testsheets/Test.xlsx")
declare -a runs=("1")
...
do
	...
	do
		...
		do
			...
			do
				OUT_FOLDER=$REPORT_HOME/async/DB/${depTableClassString[$i]}/${spreadsheetString[$j]}/RUN-${run}
				mkdir -p $OUT_FOLDER
				rm -f $OUT_FOLDER/*
				...
				now="$(date)"
				echo "$now: Finished testing ${spreadsheetString[$j]} for ${depTableClassString[$i]} using ${sheetPath}" >> out.log
				echo "" >> out.log
			done
		done
	done
