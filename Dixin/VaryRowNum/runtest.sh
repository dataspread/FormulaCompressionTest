...
TEST_HOME=/home/totemtang/dataspread/FormulaCompressionTest/Dixin/VaryRowNum
...
do
	...
	do
		for j in "${!rows[@]}"
		do
			OUT_FOLDER=$REPORT_HOME/${depTableClassString[$i]}/${rows[$j]}/RUN${run}
			mkdir -p $OUT_FOLDER
			rm -f $OUT_FOLDER/*
			...
			now="$(date)"
			echo "$now: Finished testing ${rows[$j]} for ${depTableClassString[$i]}" >> out.log
			echo "" >> out.log
		done
	done
done
