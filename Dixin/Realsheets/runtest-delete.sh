...
if [ "$#" -ne 3 ]; then
	echo "Need 3 parameters: config file, sheethome, reporthome"
    exit
fi
...
#declare -a modes=("memOnly")

while IFS=, read -r sheetname cell direction;
do
	...
	do
		...
		do
			for run in "${runs[@]}"
			do
				OUT_FOLDER=$REPORT_HOME/${depTableClassString[$i]}/${sheetname}/${modes[$j]}/RUN${run}
				mkdir -p $OUT_FOLDER
				rm -f $OUT_FOLDER/*

				memOnly="false"
				if [ "${modes[$j]}" = "memOnly" ]; then
					memOnly="true"
				fi
				...
				now="$(date)"
				echo "$now: Finished testing ${sheetname}: memOnly:${memOnly}, ${depTableClassString[$i]}" >> out.log
				echo "" >> out.log
			done
		done
	done
