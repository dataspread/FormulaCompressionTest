...
TEST_HOME=/home/totemtang/dataspread/FormulaCompressionTest/experiments_data/rq3
...
declare -a depTableClassString=("Comp" "ASync" "PGImpl")
declare -a spreadsheetString=("RefreshCache")
declare -a rows=("2000" "4000" "6000" "8000" "10000")
...
do
	...
	do
		...
		do
			for row in "${rows[@]}"
			do
				OUT_FOLDER=$REPORT_HOME/async/MEM/${depTableClassString[$i]}/${spreadsheetString[$j]}/RUN-${run}/ROWS-${row}
				mkdir -p $OUT_FOLDER
				rm -f $OUT_FOLDER/*
